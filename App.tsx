...
const App: React.FC = () => {
  ...
  return (
    ...
    // Ocupa a tela toda, dividindo espaço entre o Carrossel e o Player
    <div className="flex flex-col h-screen w-full bg-gray-900 font-sans text-white overflow-hidden">
      
      {/* Botão da Harpa (Fica sobreposto no canto superior) */}
      <button
        onClick={() => setShowHarpa(true)}
        className="absolute top-6 right-4 z-40 flex items-center justify-center w-12 h-12 rounded-full bg-black/30 backdrop-blur-md text-white border border-white/10 shadow-lg hover:bg-black/50 transition-all active:scale-95 group"
      ...
      {/* PARTE 1: Carrossel (flex-1 faz ele ocupar todo o espaço que sobrar) */}
      <div className="relative flex-1 w-full overflow-hidden">
        <HeroCarousel images={config.images} />
      </div>

      {/* PARTE 2: Player (Fixo no rodapé, tamanho original) */}
      <div className="w-full bg-gray-900 z-30 shrink-0">
         <CasterPlayer 
